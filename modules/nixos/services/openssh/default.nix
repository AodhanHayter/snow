...
let
  ...
  maximo-key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIX2R0he+RBUTv2EICttBFWGH1o5NRydkg2bJFtFG5/O aodhan.hayter@gmail.com";
...
in
{
  options.modernage.services.openssh = with types; {
    enable = mkBoolOpt false "Whether or not to enable openssh configuration.";
  ...
  config = mkIf cfg.enable {
    ...
    users.users.root.openssh.authorizedKeys.keys =
      if format == "install-iso" then cfg.authorizedKeys else [ ];
  };
}
