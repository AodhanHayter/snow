...
in
{
  options.modernage.services.kubernetes = with types; {
    ...
    role = mkOpt (enum [
      "master"
      "node"
    ]) "node" "The role this kubernetes machine should take";
    masterAddress =
      mkOpt (nullOr str) null
        "The clusterwide available network address or hostname for the kubernetes master server.";
    ...
    services.kubernetes = {
      ...
      masterAddress =
        if cfg.role == "master" then
          ...
          throw "If role is set to 'master' then a masterAddress must be configured.";
